import { feedbackUrl, metaData } from '@/lib/fetch/fetchApi'
...
export const getComments = async (params: QueryString, id: number) => {
  ...
  const res = await fetch(feedbackUrl + `/comment/${id}?${query}`)
...
export const createComment = async (params: CommentParams) => {
  ...
  const res = await fetch(feedbackUrl + '/comment', data)
...
export const BookmarkMemo = async (params: BookmarkParams) => {
  ...
  const res = await fetch(feedbackUrl + '/bookmark', data)
  if (!res.ok) throw new Error('북마크 중 에러')
  return res.json()
}

export const getAlarm = async () => {
  const res = await fetch(feedbackUrl + '/alarm')
...
export const removeAlarm = async () => {
  ...
  const res = await fetch(feedbackUrl + '/alarm', data)

...
import { signupUser } from '@/lib/fetchData'
...
export const LoginForm = () => {
   ...
   const props = useMemo(
      () => ({
         email: { error: errors.email && true, helperText: errors.email?.message || '이메일을 입력하세요.' },
         password: { error: errors.password && true, helperText: errors.password?.message || '비밀번호를 입력하세요.' },
      }),
      [errors]
   )
   ...
   return (
      <Stack spacing={1}>
         ...
         <Button variant="contained" onClick={() => socialLogin('google')}>
            구글로그인
         </Button>
         <Button variant="contained" onClick={() => socialLogin('kakao')}>
            카카오로그인
         </Button>
      </Stack>
   )
